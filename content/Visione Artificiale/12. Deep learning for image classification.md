...
| ---- | ------------ | -------------- | --------------------- |
| **LeNet-5** | Conv + average pooling + FC | Prima CNN di successo | Dimostra che l'alternanza Conv $\to$ Pool funziona per apprendere feature gerarchiche |
...
| **VGGNet** | Molti layer $3 \times 3$, max pooling, FC | Profondità con filtri piccoli | Architettura semplice e profonda, ma con moltissimi parametri |
| **GoogLeNet** | Moduli Inception + global average pooling | Inception, convoluzioni $1 \times 1$, GAP | Molto più efficiente di VGG, riduce i parametri finali |
...
**VGGNet** mostra che la profondità è un fattore chiave. Usa quasi solo convoluzioni $3 \times 3$: due convoluzioni $3 \times 3$ consecutive hanno campo ricettivo simile a una $5 \times 5$, ma con meno parametri e una non linearità in più. Il limite è il costo in memoria, dovuto soprattutto ai layer fully connected.

**GoogLeNet** punta sull'efficienza. Gli **Inception module** applicano in parallelo filtri di dimensione diversa ($1 \times 1$, $3 \times 3$, $5 \times 5$) e pooling, lasciando alla rete la scelta della scala più utile. Le convoluzioni $1 \times 1$ funzionano da bottleneck sui canali e il **global average pooling** sostituisce gran parte dei fully connected finali.

**ResNet** risolve il problema dell'addestramento di reti molto profonde. Le **skip connection** permettono al gradiente di propagarsi più facilmente e fanno sì che ogni blocco impari un residuo $\mathcal{F}(x)$ invece della trasformazione completa. Il blocco produce quindi $\mathcal{F}(x) + x$.

**MobileNet** cambia il tipo di convoluzione per ridurre il costo. Con la **depthwise separable convolution** separa il filtraggio spaziale dalla combinazione tra canali: prima un filtro per canale, poi una convoluzione $1 \times 1$ per mescolare i canali. È quindi adatta a dispositivi con risorse limitate.
